        ...
        $.ajax({
            ...
            success: function (response) {
                try {
                    ...
                    let y = 30;
                    scheduleData.data.forEach(event => {
                        doc.text(`Course: ${event.title} (${event.component_type})`, 10, y);
                        y += 10;
                        doc.text(`Day: ${event.day_of_week}`, 10, y);
                        y += 10;
                        doc.text(`Time: ${event.start_time} - ${event.end_time}`, 10, y);
                        y += 10;
                        doc.text(`Room: ${event.room}`, 10, y);
                        y += 10;
                        doc.text(`Professor: ${event.professor}`, 10, y);
                        y += 20; // Add extra space between events
                    });
